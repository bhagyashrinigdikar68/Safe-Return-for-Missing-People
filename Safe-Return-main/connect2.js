...
if (joiningDate > currentDate) {
    ...
    if (photoInput.files.length > 0) {
       ...
       try {
    const response = await fetch("http://127.0.0.1:5003/register-inmate", {
        method: "POST",
        body: formData
    });
...
window.addEventListener("DOMContentLoaded", function () {
    ...
    const dob = document.getElementById("admin-dob");
    const joining = document.getElementById("admin-joiningDate");
    if (dob) dob.max = today;
    if (joining) joining.max = today;
});
